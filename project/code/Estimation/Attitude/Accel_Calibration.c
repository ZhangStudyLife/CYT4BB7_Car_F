...
#else
    /* yaw=0: 仅 roll/pitch 旋转，得到当前姿态相对水平偏斜的重力分量 */
    r11 = cos_pitch;
...
{
    /* 防止 scale 异常（NaN/Inf/过小）时钳位到安全范围 */
    uint8_t i;
    for (i = 0U; i < 3U; i++)
    {
...
/* 陀螺仪启动函数：处理IMU校准状态机，1kHz 主循环中调用 */
