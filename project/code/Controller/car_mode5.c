...
volatile float mode5_speed_increment_limit = 6000.0f;
volatile float mode5_speed_brake_static = 800.0f;
...
volatile float mode5_gyroz_stop_target_dps = 2.0f;
...
static float s_mode5_previous_base_target;
static uint8 s_mode5_speed_brake_active;
static float s_mode5_left_brake_direction;
static float s_mode5_right_brake_direction;
...
static void mode5_speed_plan_update(void)
{
    ...
    s_mode5_previous_base_target = g_car_base_speed_target;
    if (s_mode5_large_turn_state != MODE5_LARGE_TURN_BRAKE)
    {
...
static void mode5_brake_update(uint8 command_active, float gyroz_target)
{
    if ((command_active != 0U) ||
        (fabsf(gyroz_target) >= mode5_gyroz_stop_target_dps))
    {
        s_mode5_speed_brake_active = 0U;
        return;
    }
    if ((s_mode5_speed_brake_active == 0U) &&
        ((fabsf(g_car_speed_left_filtered) > mode5_wheel_stop_speed) ||
         (fabsf(g_car_speed_right_filtered) > mode5_wheel_stop_speed)))
    {
        s_mode5_left_brake_direction = mode5_speed_direction(
            g_car_speed_left_filtered, s_mode5_previous_base_target);
        s_mode5_right_brake_direction = mode5_speed_direction(
            g_car_speed_right_filtered, s_mode5_previous_base_target);
        s_mode5_speed_brake_active = 1U;
    }
}

static float mode5_brake_feedforward(float feedback, float direction)
{
    float speed_abs = fabsf(feedback);
    float ff_deadband = fmaxf(mode5_speed_ff_deadband,
                              mode5_wheel_stop_speed + 1.0f);
    float scale;

    if ((s_mode5_speed_brake_active == 0U) || (direction == 0.0f) ||
        (feedback * direction <= 0.0f) ||
        (speed_abs <= mode5_wheel_stop_speed))
    {
        return 0.0f;
    }
    scale = (speed_abs >= ff_deadband)
                ? 1.0f
                : (speed_abs - mode5_wheel_stop_speed) /
                      (ff_deadband - mode5_wheel_stop_speed);
    return -direction * mode5_speed_brake_static * scale;
}
                                    ...
                                    float brake_ff)
{
    ...
    float ff_deadband = fmaxf(mode5_speed_ff_deadband,
                              mode5_wheel_stop_speed + 1.0f);
    float ff_transition = fmaxf(mode5_speed_ff_transition, ff_deadband);
    ...
    PID_SetStaticFeedforward(pid, mode5_speed_ff_static);
    PID_SetFeedforwardTransition(pid, ff_deadband, ff_transition);
    PID_SetExternalFeedforward(pid, 0.0f);
    PID_SetIncrementLimit(pid, mode5_speed_increment_limit);
...
void car_mode5_reset(void)
{
    ...
    s_mode5_previous_base_target = 0.0f;
    s_mode5_speed_brake_active = 0U;
    s_mode5_left_brake_direction = 0.0f;
    s_mode5_right_brake_direction = 0.0f;
    mode5_large_turn_reset();
    g_car_base_speed_command = 0.0f;
...
static void mode5_control_update(uint8 command_active,
                                 float command_speed_mps)
{
    float gyroz_target;
    float left_brake_ff;
    ...
    mode5_large_turn_update(command_active, command_speed_mps);
    ...
    {
        mode5_brake_update(command_active, gyroz_target);
    }

    large_turn_brake_ff = mode5_large_turn_brake_feedforward();
    left_brake_ff = (s_mode5_large_turn_state == MODE5_LARGE_TURN_BRAKE)
                        ? large_turn_brake_ff
                        : mode5_brake_feedforward(
                              g_car_speed_left_filtered,
                              s_mode5_left_brake_direction);
    right_brake_ff = (s_mode5_large_turn_state == MODE5_LARGE_TURN_BRAKE)
                         ? large_turn_brake_ff
                         : mode5_brake_feedforward(
                               g_car_speed_right_filtered,
                               s_mode5_right_brake_direction);
...
void car_mode5_update_100HZ(uint32 now_ms)
{
    ...
    (void)now_ms;
    command_active = mode5_world_command_update(&speed_mps);
    mode5_control_update(command_active, speed_mps);
}
