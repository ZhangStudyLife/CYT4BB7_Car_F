...
#define MODE4_LARGE_TURN_EXIT            (3U)
...
volatile float mode4_large_turn_pivot_exit_deg = 35.0f;
...
static uint16 s_mode4_large_turn_finish_cycles;
static uint16 s_mode4_large_turn_elapsed_cycles;
static float s_mode4_large_turn_target_yaw_deg;
static float s_mode4_large_turn_target_speed_mps;
...
static void mode4_large_turn_reset(void)
{
    ...
    s_mode4_large_turn_finish_cycles = 0U;
    s_mode4_large_turn_elapsed_cycles = 0U;
    s_mode4_large_turn_target_yaw_deg = 0.0f;
    s_mode4_large_turn_target_speed_mps = 0.0f;
}
...
static uint8 mode4_exit_command_matches(float heading_deg,
                                        uint8 command_active)
{
    ...
    return (fabsf(mode4_wrap_deg(
                heading_deg - s_mode4_large_turn_target_yaw_deg)) <=
            mode4_exit_command_match_deg) ? 1U : 0U;
}

static void mode4_apply_latched_command(float heading_deg,
                                        uint8 command_active)
{
    float yaw_error = mode4_wrap_deg(
        s_mode4_large_turn_target_yaw_deg - g_car_yaw_feedback_deg);
    float exit_start_deg = fmaxf(mode4_large_turn_exit_start_deg,
                                 mode4_large_turn_finish_deg + 0.5f);
    float alignment = 0.0f;

    if ((s_mode4_large_turn_state == MODE4_LARGE_TURN_EXIT) &&
        (mode4_exit_command_matches(heading_deg, command_active) != 0U) &&
        (fabsf(yaw_error) < exit_start_deg))
    {
        alignment = (exit_start_deg - fabsf(yaw_error)) /
                    (exit_start_deg -
                     mode4_large_turn_finish_deg);
        alignment = mode4_clampf(alignment, 0.0f, 1.0f);
    }

    s_mode4_yaw_target_deg = s_mode4_large_turn_target_yaw_deg;
    g_car_base_speed_command =
        car_speed_mps_to_encoder_cnt(s_mode4_large_turn_target_speed_mps) *
        alignment;
}
                                  ...
                                  uint8 command_active)
{
    ...
    if (s_mode4_large_turn_state != MODE4_LARGE_TURN_NORMAL)
    {
        ...
        {
            mode4_apply_latched_command(heading_deg, command_active);
            return 1U;
        }
    }
    ...
    yaw_error = mode4_yaw_error_deg();
    g_car_base_speed_command =
        (fabsf(yaw_error) >= mode4_alignment_stop_deg)
            ? 0.0f
            : car_speed_mps_to_encoder_cnt(speed_mps) *
                  cosf(yaw_error * MODE4_DEG_TO_RAD);
    return 1U;
}
                                    ...
                                    float command_speed_mps)
{
    ...
    uint16 finish_cycles = mode4_cycle_count(
        mode4_large_turn_finish_cycles, 1U, 100U);
    ...
    if (s_mode4_large_turn_state == MODE4_LARGE_TURN_NORMAL)
    {
            ...
            (yaw_error_abs >= mode4_large_turn_enter_deg))
        {
                ...
                trigger_cycles)
            {
                ...
                s_mode4_large_turn_target_speed_mps = command_speed_mps;
                s_mode4_large_turn_elapsed_cycles = 0U;
                s_mode4_large_turn_trigger_cycles = 0U;
    ...
    else if (s_mode4_large_turn_state == MODE4_LARGE_TURN_BRAKE)
    {
        ...
        {
            mode4_large_turn_set(MODE4_LARGE_TURN_EXIT);
        }
    ...
    else if (yaw_error_abs <= mode4_large_turn_finish_deg)
    {
        if (++s_mode4_large_turn_finish_cycles >=
            finish_cycles)
        {
            mode4_large_turn_set(MODE4_LARGE_TURN_NORMAL);
        }
